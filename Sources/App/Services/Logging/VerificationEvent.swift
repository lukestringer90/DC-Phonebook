...
enum VerificationEvent: Event {
    ...
    func message() -> String {
        ...
        case .started(let applicantID,  let date):
            return "\(date.asLogFormat): \(applicantID.asTaggedMessage) **STARTED** verification process using `\(Constants.Discord.VerifyStartMessage.commandString)`"
        case .startedDuplicate(let applicantID, let date):
            return "\(date.asLogFormat): \(applicantID.asTaggedMessage) used `\(Constants.Discord.VerifyStartMessage.commandString)` without completing verification process"
        }
    }
}
